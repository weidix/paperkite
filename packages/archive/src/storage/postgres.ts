...
export class PostgresArchiveStore implements ArchiveStore {
    ...
    try {
      ...
      await client.query(`
        ...
        CREATE INDEX IF NOT EXISTS idx_messages_forward_from
          ON ${this.table("messages")} (forward_from_id);
      ...
      `);
      await this.ensureLegacyColumns(client);
  ...
  private async ensureLegacyColumns(client: PoolClient): Promise<void> {
    ...
    const names = new Set(exists.rows.map((row) => String(row.column_name)));
  ...
  async saveBatch(messages: readonly MessageRow[], media: readonly MediaRow[]): Promise<BatchResult> {
    ...
    try {
      ...
      const messagePlaceholders = messages
        .map((_, index) => `(${paramPlaceholders(index * 20 + 1, 20)})`)
        .join(", ");
  ...
  async searchSenders(query: SenderQuery): Promise<SenderSearchResult> {
    const limit = normalizeLimit(query.limit);
    const conditions = ["m.blocked = FALSE"];
    const values: unknown[] = [];
    const q = (query.q ?? "").trim().replace(/^@/, "");
    if (q) {
      conditions.push(`(
        m.sender_id = $${values.length + 1}
        OR m.sender_id ILIKE $${values.length + 2}
        OR m.sender_username ILIKE $${values.length + 3}
        OR m.sender_first_name ILIKE $${values.length + 4}
        OR m.sender_last_name ILIKE $${values.length + 5}
      )`);
      const like = `%${escapeLike(q)}%`;
      values.push(q, like, like, like, like);
    }
    if (query.chatId?.trim()) {
      conditions.push(`m.chat_id = $${values.length + 1}`);
      values.push(query.chatId.trim());
    }
    const where = `WHERE ${conditions.join(" AND ")}`;
    const total = await this.pool.query(
      `SELECT COUNT(DISTINCT m.sender_id)::int AS count FROM ${this.table("messages")} m ${where}`,
      values
    );
    const rows = await this.pool.query(
      `SELECT m.sender_id,
              MAX(COALESCE(m.sender_username, ''))::text AS sender_username,
              MAX(COALESCE(m.sender_first_name, ''))::text AS sender_first_name,
              MAX(COALESCE(m.sender_last_name, ''))::text AS sender_last_name,
              COUNT(*)::int AS count
         FROM ${this.table("messages")} m
         ${where}
    ...
    return {
      items: rows.rows.filter((row) => row.sender_id !== null && row.sender_id !== "").map(toSenderInfo),
  ...
  async getSenderSummary(senderId: string, chatId?: string): Promise<SenderSummary | undefined> {
    ...
    const exists = await this.pool.query(
      `SELECT 1 FROM ${this.table("messages")} WHERE sender_id = $1 LIMIT 1`,
      [id]
    );
    if (exists.rows.length === 0) return undefined;
    const chatFilter = chatId?.trim() ? " AND m.chat_id = $2" : "";
    const values = chatId?.trim() ? [id, chatId.trim()] : [id];
    const stats = await this.pool.query(
      `SELECT COUNT(*)::int AS total, MIN(m.date) AS first_date, MAX(m.date) AS last_date
         FROM ${this.table("messages")} m
        WHERE m.sender_id = $1 AND m.blocked = FALSE${chatFilter}`,
      values
    );
    const chatRows = await this.pool.query(
      `SELECT m.chat_id, c.title AS chat_title, COUNT(*)::int AS count,
              MAX(m.date) AS last_date,
              (
                SELECT LEFT(m2.text, 120)
                  FROM ${this.table("messages")} m2
                 WHERE m2.chat_id = m.chat_id AND m2.sender_id = m.sender_id AND m2.blocked = FALSE
                 ORDER BY m2.date DESC, m2.id DESC LIMIT 1
              ) AS last_text
         FROM ${this.table("messages")} m
         LEFT JOIN ${this.table("chats")} c ON c.chat_id = m.chat_id
        WHERE m.sender_id = $1 AND m.blocked = FALSE${chatFilter}
    ...
    return {
      sender: toSenderInfo(identity.rows[0] ?? { sender_id: id }),
      total: Number(statRow?.total ?? 0),
      firstDate: statRow?.first_date === null || statRow?.first_date === undefined ? undefined : toIsoDate(statRow.first_date),
      lastDate: statRow?.last_date === null || statRow?.last_date === undefined ? undefined : toIsoDate(statRow.last_date),
      chats: chatRows.rows.map((row) => ({
        chatId: String(row.chat_id),
  ...
  async getReplyChain(recordId: string): Promise<ReplyChainResult | undefined> {
    ...
    const childRows = await this.pool.query(
      `${messageSelect(this.table("messages"), this.table("media_files"))}
       WHERE m.chat_id = $1 AND m.reply_to_msg_id = $2::bigint AND m.blocked = FALSE
       ORDER BY m.date ASC, m.id ASC
       LIMIT 100`,
      [anchor.chatId, anchor.messageId]
    );
    const children = childRows.rows.length
      ? buildContextEntries(
          await this.attachMedia(childRows.rows),
          await this.fetchGroupsAttached(uniqueGroupKeys(childRows.rows))
        )
      : [];
    return {
      parent,
...
function buildWhere(query: ArchiveQuery, alias = "m"): { where: string; values: string[] } {
  ...
  if (chatIds.length > 0) {
    ...
    values.push(...chatIds);
    conditions.push(`${alias}.chat_id IN (${paramPlaceholders(start, chatIds.length)})`);
  }
  ...
  if (senderIds.length > 0) {
    ...
    conditions.push(`${alias}.sender_id IN (${paramPlaceholders(start, senderIds.length)})`);
  }
  if (query.forwardFromId?.trim()) add(`${alias}.forward_from_id = $N`, query.forwardFromId.trim());
  if (query.forwardFromName?.trim()) add(`${alias}.forward_from_name ILIKE $N`, `%${escapeLike(query.forwardFromName.trim())}%`);
...
function toSenderInfo(row: QueryResultRow): SenderInfo {
  return {
